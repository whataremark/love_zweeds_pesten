...
function browser.keypressed(key)
    ...
    elseif key == "i" then
        browser.typing = true
        browser.manualIP = browser.manualIP
    elseif key == "escape" then
...
function browser.draw()
    ...
    local panelW = math.min(660, w * 0.72)
    local panelH = math.min(500, h * 0.76)
    ...
    if #browser.hosts == 0 then
        ...
        for index, host in ipairs(browser.hosts) do
            local rowY = listTop + (index - 1) * (rowH + 12)
            ...
            if isSelected then
                ...
                love.graphics.setColor(0.11, 0.22, 0.17, 0.7)
            end
            love.graphics.rectangle("fill", listLeft, rowY, listRight - listLeft, rowH, 18, 18)

            love.graphics.setColor(1, 1, 1, isSelected and 0.92 or 0.72)
            love.graphics.printf(host.name or "Unknown host", listLeft + 20, rowY + 6, listRight - listLeft - 40, "left")

            love.graphics.setColor(1, 1, 1, isSelected and 0.85 or 0.55)
            love.graphics.printf(host.ip or "?", listLeft + 20, rowY + 22, listRight - listLeft - 40, "left")
        end
    end

    local manualTop = listTop + math.max(1, #browser.hosts) * (rowH + 12) + 24
    local manualHeight = 54

    love.graphics.setColor(1, 1, 1, 0.7)
    love.graphics.setFont(bodyFont)
    love.graphics.printf("Manual IP", listLeft, manualTop - 32, listRight - listLeft, "left")

    if browser.typing then
        love.graphics.setColor(0.18 + pulse * 0.25, 0.48 + pulse * 0.3, 0.24 + pulse * 0.22, 0.98)
    else
        love.graphics.setColor(0.12, 0.2, 0.16, 0.7)
    end
    love.graphics.rectangle("fill", listLeft, manualTop, listRight - listLeft, manualHeight, 16, 16)

    love.graphics.setFont(bodyFont)
    love.graphics.setColor(1, 1, 1, 0.88)
    local caret = browser.typing and ((math.floor((browser.time or 0) * 2) % 2 == 0) and "_" or "") or ""
    local ipText = browser.manualIP
    if not browser.typing and #ipText == 0 then
        ipText = "Press I to enter an IP"
        love.graphics.setColor(1, 1, 1, 0.5)
        caret = ""
    end
    love.graphics.printf(ipText .. caret, listLeft + 20, manualTop + 16, listRight - listLeft - 40, "left")

    love.graphics.setFont(smallFont)
    love.graphics.setColor(1, 1, 1, 0.62)
    love.graphics.printf("Use ↑ ↓ to pick a lobby • Enter to join • I to type an IP\nEsc to return to the menu", panelX, panelY + panelH - 80, panelW, "center")
end
